...
> - 8×400G 节点(轨数=rank数):block=1 → rank i → `ib7s400p{i}`(p0-3=NUMA0/GPU0-3、p4-7=NUMA1/GPU4-7,逐 rank 一轨)。
> - 2×400G 节点(轨<rank):block=4 → ranks 0-3 → 轨0(NUMA0)、ranks 4-7 → 轨1(NUMA1),每轨 4 个 rank。(取模会把同 socket 的 rank 拆到两个 NUMA 轨,故用分块。)
