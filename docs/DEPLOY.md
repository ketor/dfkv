...
> **多 TP rank 多轨亲和**：SGLang 各 rank 共享一份 `DFKV_RDMA_DEV`。设 extra_config `"rail_affinity":true` 后，每个 rank 按 **NUMA 分块**收窄到一条轨：`block=tp_size/轨数`、`轨 = rails[tp_rank // block]`。让**所有读 rank 确定性铺满可用轨且 NUMA 本地**，自动 `DFKV_RDMA_NUMA=1` 绑缓冲/线程到该轨 NUMA。**仅提升读聚合带宽**；写是 MLA rank0 单进程不受影响。**要求 `DFKV_RDMA_DEV` 按 NUMA 顺序列设备(NUMA0 的口在前)**。
> - hd03(8×400G,轨数=rank数):block=1 → rank i → `ib7s400p{i}`(p0-3=NUMA0/GPU0-3、p4-7=NUMA1/GPU4-7,逐 rank 一轨)。
> - hd04(2×400G,轨<rank):block=4 → ranks 0-3 → 轨0(NUMA0)、ranks 4-7 → 轨1(NUMA1),每轨 4 个 rank。(取模会把同 socket 的 rank 拆到两个 NUMA 轨,故用分块。)
