...
Subpackages:
  ...
  tracing       connector-side request tracing (slow / sampled / failed spans),
                pushed over OTLP /v1/traces

Usage in a connector __init__:
    from dfkv_telemetry import metrics, tracing, config
    metrics.configure(extra_cfg, connector_type=config.TYPE_VLLM,
                      tp_rank=rank, model=model_name)
    tracing.configure(extra_cfg, connector_type=config.TYPE_VLLM,
                      tp_rank=rank, model=model_name)

Usage at an op chokepoint:
    with metrics.op("get", num_keys=n, num_bytes=nbytes) as m, \
            tracing.span("get", num_keys=n) as sp:
        ... do the C call ...
        m.keys = hits          # optional: refine keys/bytes after the call
        sp.hits = hits         # optional: span attributes
        # status defaults to 'fail' if the block raised, else 'ok'
"""

from __future__ import annotations

from . import config, metrics_push, tracing
from . import metrics_push as metrics  # friendly alias: dfkv_telemetry.metrics

__all__ = ["config", "metrics", "metrics_push", "tracing"]
