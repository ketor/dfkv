...
bool RdmaCmTransport::Available() {
  int n = 0;
...
RdmaCmTransport::RdmaCmTransport(size_t max_msg) : max_msg_(max_msg) {}

RdmaCmTransport::~RdmaCmTransport() {
  std::lock_guard<std::mutex> lk(mu_);
  for (auto& [node, cs] : pool_)
    for (Conn* c : cs) Destroy(c);
}

void RdmaCmTransport::Destroy(Conn* c) {
  if (!c) return;
...
RdmaCmTransport::Conn* RdmaCmTransport::Acquire(const std::string& node, bool* from_pool) {
  {
    std::lock_guard<std::mutex> lk(mu_);
...
void RdmaCmTransport::Release(const std::string& node, Conn* c) {
  std::lock_guard<std::mutex> lk(mu_);
  pool_[node].push_back(c);
}

Status RdmaCmTransport::RoundTrip(const std::string& node, WireOp op,
...
Status RdmaCmTransport::Cache(const std::string& node, const BlockKey& key,
                            const void* data, size_t len) {
  return RoundTrip(node, WireOp::kCache, key, 0, 0, data, len, nullptr);
}
Status RdmaCmTransport::Range(const std::string& node, const BlockKey& key,
                            uint64_t offset, uint64_t length, std::string* out) {
  return RoundTrip(node, WireOp::kRange, key, offset, length, nullptr, 0, out);
}
Status RdmaCmTransport::Exist(const std::string& node, const BlockKey& key, bool* exist) {
